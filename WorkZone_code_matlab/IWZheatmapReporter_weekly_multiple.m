...
metalist = dir(x);
metalist(1:2) = [];
dirFlags = [metalist.isdir];
% Extract only those that are directories.
subFolders = metalist(dirFlags);
% Print folder names to command window.
for k = 11 : length(subFolders)
	fprintf('Sub folder #%d = %s\n', k, subFolders(k).name);

    reportstartday = subFolders(k).name(1:10); 
    reportendday = subFolders(k).name(12:end);


    y = ['S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2017\Weekly Reports\' reportstartday '-' reportendday];
    % rmdir(y);
    mkdir(y);

    list1 = dir([x '\' subFolders(k).name '\IWZ\*.txt']);
    list2 = dir([x '\*.csv']);
    ...
    %,'27.95 percentile of delay for all times','28.95 percentile of delay when there is delay'};
...
%     IWZ = 1:8;
    ...
    %i=6;
        ...
        [speeddata,csvdata] = IWZread([x '\' subFolders(k).name '\IWZ\' list1(i).name]);
    ...
    csvfilename='S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2017\Tableau\performance_table-2017.csv';
    % csvfilename='S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2016\Weekly Reports\performance_table.csv';
    nrows = size(performanceTable,1)-1;
    fid = fopen(csvfilename,'a');
    for aa = 1 : nrows
        fprintf(fid,'%s,%s,%s,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\r\n'...
            ,'ID',[reportstartday ' to ' reportendday],performanceTable{aa+1,1},performanceTable{aa+1,2}...
            ,performanceTable{aa+1,3},performanceTable{aa+1,4},performanceTable{aa+1,5},performanceTable{aa+1,6}...
            ,performanceTable{aa+1,7},performanceTable{aa+1,8},performanceTable{aa+1,9},performanceTable{aa+1,10}...
            ,performanceTable{aa+1,11},performanceTable{aa+1,12},performanceTable{aa+1,13},performanceTable{aa+1,14}...
            ,performanceTable{aa+1,15}/7,performanceTable{aa+1,16},performanceTable{aa+1,17},performanceTable{aa+1,18}...
            ,performanceTable{aa+1,19},performanceTable{aa+1,20},performanceTable{aa+1,21},performanceTable{aa+1,22}...
            ,performanceTable{aa+1,23},performanceTable{aa+1,24},performanceTable{aa+1,25},performanceTable{aa+1,26}...
            ,performanceTable{aa+1,27},performanceTable{aa+1,28});
    end
    fclose(fid);
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% performance summary 2017
    fid = fopen('S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2017\Tableau\performance_table-2017.csv');
    ...
    fid1 = fopen('S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2017\Tableau\performance_table_summary-2017.csv','w');
    ...
    for i=1:numel(header)
        fprintf(fid1,'%s,%s,%s,%s,%s,%s,',['min_' header{i}],['max_' header{i}],...
            ['median_' header{i}],['std_' header{i}],['25prctile_' header{i}],['75prctile_' header{i}]);
    end
    fprintf(fid1,'\r\n');
    for i=1:max(uniqueindex)
        ...
        for k=1:numel(data_min)
            fprintf(fid1,'%f,%f,%f,%f,%f,%f,',data_min(k),data_max(k),data_median(k),...
                data_std(k),data_25(k),data_75(k));
        end
        fprintf(fid1,'\r\n');
    end
    fclose(fid1);