function figurenum=dailysensorissue_monthly(TrafficData,startdatenum,figurenum,milemarker,direction,groupname,reportstartday,sensornames)
...
if size(TrafficData,1)<13  
set(gca,'YTick',1:size(TrafficData,1))
else set(gca,'YTick',1:2:size(TrafficData,1))
end
...
set(gca,'XGrid','on')
    ...
    issues={'normal','off','fail','zerospeednonzerocount','missingveh','lose connection'};
    for j=1:levels
    ...
    caxis([0,50]);
    hcb=colorbar;
 set(hcb,'YTick',[0,10,20,30,40,50])
 set(hcb,'YTickLabel',{'','','','','',''})
