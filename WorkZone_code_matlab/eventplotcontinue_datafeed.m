% event plot continue
function [figurenum]...
    =eventplotcontinue_datafeed(eventamplifier,TrafficData,volume,figurenum,eventspeed,...
...
%             plot(y(brk+1:end),xedge(brk+1:end),cell2mat(colorscale(dayofweek)),'LineWidth',3,'Marker','.','MarkerSize',10)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% write shockwaves into csv. Disable when
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% unwanted
    for i=1:size(cellevent,2)
        ...
        csvfilename=['S:\(S) SHARE\_project CTRE\1_Active Research Projects\Iowa DOT OTO Support\14_Traffic Critical Projects 2\2016\IWZ Data for Tableau\' groupname '_shockwave.csv'];
        nrows = size(event,2);
        fid = fopen(csvfilename,'a');
        for aa = 1 : nrows
            if overnight==0
                fprintf(fid,'%s,%f,%f,%s,%s,%f,%f,%s,%s,%f,%f\r\n', [reportstartday '-' reportendday]...
                    ,i,y(aa),[topeventday{i} ' ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],...
                    ['1/1/2001 ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],xedge(aa)...
                    ,(xedge(aa)-1)*0.1+milemarker(1),[topeventday{i} ' ' topeventstarttime{i}]...
                    ,dayofweek,direction,eventseverity(i));
            else if aa<=brk
                    fprintf(fid,'%s,%f,%f,%s,%s,%f,%f,%s,%s,%f,%f\r\n', [reportstartday '-' reportendday]...
                    ,i,y(aa),[topeventday{i} ' ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],...
                    ['1/1/2001 ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],xedge(aa)...
                    ...
                    fprintf(fid,'%s,%f,%f,%s,%s,%f,%f,%s,%s,%f,%f\r\n', [reportstartday '-' reportendday]...
                    ,i,y(aa),[datestr(1+startdatenum+floor(event(end-1,1)/(24*60/timeinterval))) ' ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],...
                    ['1/1/2001 ' num2str(floor(y(aa)/12)) ':' num2str(mod(y(aa),12)*5)],xedge(aa)...
                    ,(xedge(aa)-1)*0.1+milemarker(1),[topeventday{i} ' ' topeventstarttime{i}]...
                    ,dayofweek,direction,eventseverity(i));
                end
